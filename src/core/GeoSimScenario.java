...
public class GeoSimScenario implements Serializable {
	...
	/** update interval for checking the positions of the hosts -setting id ({@value}) */
	public static final String PC_INT_S = "positionCheckInterval";
	...
	/** Update interval for checking the positions of the hosts */
	...
	protected GeoSimScenario() {
		...
		this.positionCheckInterval = s.getDouble(PC_INT_S);
		...
		int[] zoneSize = s.getCsvInts(MovementModel.ZONE_SIZE, 2);
		this.zoneSizeX = zoneSize[0];
		this.zoneSizeY = zoneSize[1];
		this.worldSizeX = worldSize[0];
		this.worldSizeY = worldSize[1];
		
		createHosts();
		
		this.world = new GeoWorld(hosts, worldSizeX, worldSizeY, 
				zoneSizeX, zoneSizeY, positionCheckInterval, updateInterval, 
